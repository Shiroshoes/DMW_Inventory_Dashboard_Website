...
$role = $_SESSION['role']; // "Admin" or "Cashier"

// Role-based filter
$user_filter = $role === 'Cashier' ? "WHERE user_id = ?" : "WHERE 1";

//  KPI totals
$stmt = $pdo->prepare("SELECT SUM(total) AS total_sales, SUM(quantity) AS total_qty 
                       FROM daily_records 
                       $user_filter AND DATE_FORMAT(record_date,'%Y-%m')=?");
$stmt->execute($role === 'Cashier' ? [$user_id, $month] : [$month]);
$totals = $stmt->fetch(PDO::FETCH_ASSOC);

// Top category
$stmt = $pdo->prepare("SELECT category, SUM(total) AS cat_total 
                       FROM daily_records 
                       $user_filter AND DATE_FORMAT(record_date,'%Y-%m')=? 
                       GROUP BY category 
...
$stmt = $pdo->prepare("SELECT record_date, category, SUM(total) AS daily_total 
                       FROM daily_records 
                       $user_filter AND DATE_FORMAT(record_date,'%Y-%m')=? 
...
foreach ($dates as $d) {
  foreach ($cats as $c) {
    $cat_data[$c][] = 0;
    foreach ($rows as $r) {
      if ($r['record_date'] == $d && $r['category'] == $c) {
        $cat_data[$c][count($cat_data[$c]) - 1] = $r['daily_total'];
      }
    }
  }
}
...
$stmt = $pdo->prepare("SELECT item_name, category, SUM(total) AS total_sale 
                       FROM daily_records 
                       $user_filter AND DATE_FORMAT(record_date,'%Y-%m')=? 
...
<head>
  ...
  <link rel="stylesheet" href="/website/tropangmaselan-motorshop-main/php/Admin/dashboard/dashboard.css" />
